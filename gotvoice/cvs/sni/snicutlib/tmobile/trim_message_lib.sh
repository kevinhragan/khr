...
trim_message()
{
	...
	snicut_output="`/usr/local/bin/snicut -l${_TM_SNI_PROVIDER_LIB} ${_TM_ORIG_MESSAGE} -BamTmo,pmTmo 2>&1`"
	...
	#Determine success.
	if [ "${_TM_NEW_FILE_SIZE}" -gt "60" -a "${_TM_FILE_SIZE_DIFF}" -gt "0" -a "${_TM_FILE_SIZE_DIFF}" -lt "280000" ] ; then
	    ...
	    if [ "${_TM_FILE_SIZE_DIFF}" -lt "0" -o "${_TM_FILE_SIZE_DIFF}" -gt "280000" ] ; then
